...
# 以下、jf-dvr ブリッジ用の追加
# KonomiTV では接続先 URL を app.config.Config 経由で取得していたが、
# 本パッケージ単体で完結させるためモジュールグローバルに保持する

_edcb_url = None


def set_edcb_url(url) -> None:
    """ EDCB (EpgTimerSrv) の接続先 URL (pydantic_core.Url) を設定する """
    global _edcb_url
    _edcb_url = url


def get_edcb_url():
    """ 設定済みの EDCB (EpgTimerSrv) 接続先 URL を返す (未設定なら None) """
    return _edcb_url
