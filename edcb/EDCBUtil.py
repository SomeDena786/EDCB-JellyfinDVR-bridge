...
from . import ChSet5Item, get_edcb_url
from .CtrlCmdUtil import CtrlCmdUtil
from .PipeStreamReader import PipeStreamReader
...
class EDCBUtil:
    ...
    def getEDCBHost(edcb_url: Url | None = None) -> str | None:
        ...
        """
        if edcb_url is None:
            edcb_url = get_edcb_url()
        return edcb_url.host if edcb_url is not None else None
    ...
    def getEDCBPort(edcb_url: Url | None = None) -> int | None:
        ...
        if edcb_url is None:
            edcb_url = get_edcb_url()
        return edcb_url.port if edcb_url is not None else None
